...
class ticket extends web {
	...
	public function lists() {
		...
		switch ( $_GET['status'] ) {
			case 1:
			case 2:
				$sql = "SELECT t.tid,count(t.tid) as nums,t.discount,t.condition,t.tid,t.starttime,t.endtime,t.title,t.limit,t.thumb,tr.status,tr.uid,tr.remark FROM " . tablename( 'ticket' ) . " t JOIN " . tablename( 'ticket_record' ) . " tr ON t.tid=tr.tid " . "WHERE tr.uid={$_SESSION['user']['uid']} AND t.type={$_GET['type']} AND tr.status={$_GET['status']} GROUP BY t.tid";
				break;
			case 3:
				//过期
				$sql = "SELECT t.tid,count(t.tid) as nums,t.discount,t.condition,t.tid,t.starttime,t.endtime,t.title,t.limit,t.thumb,tr.status,tr.uid,tr.remark FROM " . tablename( 'ticket' ) . " t JOIN " . tablename( 'ticket_record' ) . " tr ON t.tid=tr.tid " . "WHERE tr.uid={$_SESSION['user']['uid']} AND t.type={$_GET['type']} AND t.endtime<" . time() . " GROUP BY t.tid";
		}
	...
	public function convert() {
		if ( IS_POST ) {
			...
			//会员已经兑换的数量
			$count = Db::table( 'ticket_record' )->where( 'tid', $tid )->where( 'uid', $_SESSION['user']['uid'] )->count();
			//卡券信息
			...
			//减掉会员积分
			Db::table( "member" )->where( 'uid', $_SESSION['user']['uid'] )->decrement( 'credit1', $ticket['credit'] );
			//增加会员卡券记录
			$data['tid']        = $ticket['tid'];

...
<block name="content">
    ...
    <form action="" method="post" ng-controller="ctrl" class="ng-cloak" ng-cloak ng-submit=" submit($event)">
            ...
            <div class="panel-body">
                <div ng-repeat="m in v._data">
                    <p class="col-sm-2 col-md-2 col-lg-2" ng-repeat="d in m._data">
                        <label>
                            <input type="checkbox" name="system[]" value="@{{d.permission}}" ng-checked="d._status">
                            @{{d.title}}
                        </label>
                    </p>
                </div>
            ...
            <div class="panel-body">
                <div ng-repeat="t in v['access']">
                    <p class="col-sm-2 col-md-2 col-lg-2" ng-repeat="d in t">
                        <label>
                            <input type="checkbox" name="modules[@{{v.module.name}}][]" value="@{{d.identifying}}"
                                   ng-checked="d._status">
                            @{{d.title}}
                        </label>
                    </p>
                </div>
        ...
        <div class="btn-group" role="group" aria-label="...">
            <button type="button" class="btn btn-default" ng-click="cancelSelectAllCheckbox()">取消选择</button>
            <button type="button" class="btn btn-default" ng-click="selectAllCheckbox()">选择全部</button>
...
<script>
    require(['angular', 'underscore', 'util', 'jquery', 'hdcms'], function (angular, _, util, $, hdcms) {
        angular.module('app', []).controller('ctrl', ['$scope', '$sce', function ($scope, $sce) {
            ...
            //选择所有表单
            $scope.cancelSelectAllCheckbox = function () {

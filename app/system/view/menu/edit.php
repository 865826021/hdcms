			...
			<div class="panel-body table-responsive">
				<table class="table">
					...
					<tr>
						...
						<td>
							<div ng-class="{pill:v._level>1}" ng-style="v._level==3?{'margin-left': 50}:''">
								<input type="text" ng-model="v.title" class="form-control" value="基础设置"
								       placeholder="@{{v.placeholder}}" style="width:150px;">
							</div>
						</td>
						<td>
						...
						<td>
							<div class="input-group" ng-if="v._level==1">
								...
								<span class="input-group-btn">
                                    <button class="btn btn-default ico-modal" type="button" ng-click="changeIcon(v)">图标</button>
                                </span>
							</div>
							<span ng-if="!v.new_add && v._level!=1" ng-bind="v.permission"></span>
							<input type="text" ng-if="v.new_add && v._level==3" ng-model="v.permission"
							       class="form-control" placeholder="权限标识">
						</td>
						<td>
							<input type="text" ng-if="v.new_add && v._level==3" ng-model="v.url" class="form-control"
							       placeholder="链接地址">
						</td>
						<td>
							<span class="btn btn-success" title="点击切换" ng-click="changeDisplayStatus(v.id,0)"
							      ng-if="v.is_display==1 && !v.new_add">显示</span>
							<span class="btn btn-danger" title="点击切换" ng-click="changeDisplayStatus(v.id,1)"
							      ng-if="v.is_display==0 && !v.new_add">隐藏</span>
						</td>
						<td>
						...
						<td>
							<input type="text" ng-if="v.new_add && v._level==3" ng-model="v.append_url"
							       class="form-control" placeholder="子链接地址">
						</td>
						<td>
							<span class="btn btn-danger" ng-if="!v.new_add && v.is_system==0" title="删除菜单"
							      ng-click="delMenu(v)">删除</span>&nbsp;
							<i class="fa fa-times-circle" ng-if="v.new_add" style="cursor: pointer"
							   onclick="$(this).parents('tr').eq(0).remove();"></i>
						</td>
					</tr>
					...
					<tr>
						<td>&nbsp;</td>
						<td colspan="8">
							<div class="add add_level0" style="cursor: pointer;" ng-click="addMenu(v,1)">
...
//			});
			...
			$scope.changeDisplayStatus = function (id, state) {
				$.post("{{u('changeDisplayState')}}", {id: id, is_display: state}, function (json) {
					util.message(json.message,'refresh','message');
				},'json')
			}
			//添加菜单
